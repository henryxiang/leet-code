...
query="{\"operationName\":\"questionData\",\"variables\":{\"titleSlug\":\"$1\"},\"query\":\"query questionData(\$titleSlug: String\u0021) { question(titleSlug: \$titleSlug) { questionId questionFrontendId boundTopicId title titleSlug content isPaidOnly difficulty likes dislikes isLiked similarQuestions contributors { username profileUrl avatarUrl } langToValidPlayground topicTags { name slug } companyTagStats codeSnippets { lang code } stats hints solution { id canSeeDetail } status sampleTestCase metaData judgerAvailable judgeType mysqlSchemas enableRunCode enableTestMode libraryUrl }}\"}"
